...
describe("E2E tests", function () {
  ...
  describe("Navigation bar [ 10 Points ]", () => {
    ...
    it("Guest user should see correct navigation [ 2.5 Points ]", async function () {
      ...
      expect(await page.isVisible("nav"), "Dashboard is not visible").to.be
        .true;
      expect(await page.isVisible("nav >> text=Add Show"), "Create is visible")
        .to.be.false;
    ...
    it("Guest user navigation should work [ 2.5 Points ]", async function () {
      ...
      let tvShowBtn = await page.waitForSelector("nav >> text=TV Shows", {
        timeout: interval,
      });
    ...
    it("Logged in user navigation should work [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("#shows", { timeout: interval });
      let createBtn = await page.waitForSelector("nav >> text=Add Show", {
        timeout: interval,
      });
  ...
  describe("Home Page [ 5 Points ]", function () {
    it("Show Home page text [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector(
        "text=Welcome to ShowShare, the ultimate platform for TV enthusiasts! Discover, discuss, and share your favorite TV shows with a community that loves great content just as much as you do. Find hidden gems. Your next binge-worthy series is just a click away!",
        { timeout: interval }
      );
      expect(
  ...
  describe("Dashboard Page [ 15 Points ]", function () {
    it("Show TV Shows page - welcome message [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("main >> text=Users Recommendations", {
        timeout: interval,
      });
      expect(await page.isVisible("main >> text=Users Recommendations")).to.be
        .true;
    });
    ...
    it("Check Shows have correct images [ 2.5 Points ]", async function () {
      ...
      const images = await page.$$eval(".show img", (t) => t.map((s) => s.src));
    ...
    it("Check Shows have Details button [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector(".show >> text=Details", {
        timeout: interval,
      });
  ...
  describe("CRUD [ 50 Points ]", () => {
    describe("Create [ 12.5 Points ]", function () {
      it("Create does NOT work with empty fields [ 2.5 Points ]", async function () {
        ...
        let addCharacterBtn = await page.waitForSelector("text=Add Show", {
          timeout: interval,
        });
      ...
      it("Create makes correct API call [ 2.5 Points ]", async function () {
        ...
        let genreElement = await page.waitForSelector('[name="genre"]', {
          timeout: interval,
        });
        let countryElement = await page.waitForSelector('[name="country"]', {
          timeout: interval,
        });
        let detailsElement = await page.waitForSelector('[name="details"]', {
          timeout: interval,
        });
        let submitBtn = await page.waitForSelector('[type="submit"]', {
          timeout: interval,
    ...
    describe("Details [ 10 Points ]", function () {
      it("Details calls the correct API [ 2.5 Points ]", async function () {
        ...
        let learnMoreButton = await page.waitForSelector(
          `.show:has-text("${data.title}") >> .details-btn`,
          { timeout: interval }
        );
      ...
      it("Details with guest calls shows correct info [ 2.5 Points ]", async function () {
        ...
        let detailsElement = await page.waitForSelector("#description", {
          timeout: interval,
        });
    ...
    describe("Edit [ 17.5 Points ]", function () {
      it("Edit calls correct API to populate info [ 2.5 Points ]", async function () {
        ...
        let learnMoreButton = await page.waitForSelector(
          `.show:has-text("${data.country}") >> .details-btn`,
          { timeout: interval }
        );
        await learnMoreButton.click();
      ...
      it("Edit does NOT work with empty fields [ 2.5 Points ]", async function () {
        ...
        let typeInput = await page.waitForSelector('[name="title"]', {
          timeout: interval,
        });
        let imageInput = await page.waitForSelector('[name="image-url"]', {
          timeout: interval,
        });
        let descriptionInput = await page.waitForSelector('[name="genre"]', {
          timeout: interval,
        });
        let learnMoreInput = await page.waitForSelector('[name="country"]', {
          timeout: interval,
        });
      ...
      it("Edit sends correct information [ 2.5 Points ]", async function () {
        ...
        let countryInput = await page.waitForSelector('[name="country"]', {
          timeout: interval,
        });
  ...
  describe("BONUS:Search Page [ 15 Points ]", async () => {
    it("Show no matches for Guest [ 2.5 Points ]", async () => {
      ...
      expect(await page.isVisible("text=There is no TV show with this title"))
        .to.be.true;
    });
