...
const mockData = {
  ...
  catalog: [
    ...
    },
    {
      ...
      learnMore:
        "Hyperaccumulating plants are species that have evolved the ability to accumulate high concentrations of heavy metals and other pollutants in their tissues without being harmed. By planting these hyperaccumulators in contaminated sites, pollutants can be extracted from the soil and water and stored in the plant biomass. Once the plants have absorbed the contaminants, they can be harvested and disposed of properly, effectively detoxifying the environment. Additionally, genetic engineering techniques can be employed to enhance the hyperaccumulation capabilities of plants, making them even more effective at cleaning up polluted sites.",
...
const endpoints = {
  ...
  own: (solutionId, userId) =>
    `/data/solutions?where=_id%3D%22${solutionId}%22%20and%20_ownerId%3D%22${userId}%22&count`,
  like: "/data/likes",
  totalLikes: (solutionId) =>
    `/data/likes?where=solutionId%3D%22${solutionId}%22&distinct=_ownerId&count`,
  userLikes: (solutionId, userId) =>
...
describe("E2E tests", function () {
  ...
  describe("Navigation bar [ 10 Points ]", () => {
    it("Logged user should see correct navigation [ 2.5 Points ]", async function () {
      ...
      //Test for navigation
      await page.waitForSelector("nav >> text=Solutions", {
        timeout: interval,
      });
    ...
    it("Guest user should see correct navigation [ 2.5 Points ]", async function () {
      ...
      expect(await page.isVisible("nav"), "Dashboard is not visible").to.be
        .true;
      expect(
        await page.isVisible("nav >> text=Add Solution"),
        "Create is visible"
      ).to.be.false;
    ...
    it("Guest user navigation should work [ 2.5 Points ]", async function () {
      ...
      let solutionsBtn = await page.waitForSelector("nav >> text=Solutions", {
        timeout: interval,
      });
    ...
    it("Logged in user navigation should work [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("#solutions", { timeout: interval });
      let createBtn = await page.waitForSelector("nav >> text=Add Solution", {
        timeout: interval,
      });
  ...
  describe("Home Page [ 5 Points ]", function () {
    it("Show Home page text [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector(
        "text=Dive into our world of cutting-edge technologies and out-of-the-box ideas designed to make Mother Nature smile again. From quirky gadgets to serious solutions, we're here to show you that saving the planet can be as exciting as it is essential. Join us in our mission to turn \"green\" into more than just a color - it's a lifestyle!",
        { timeout: interval }
      );
      expect(
  ...
  describe("Dashboard Page [ 15 Points ]", function () {
    it("Show Solutions page - welcome message [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("main >> text=Solutions", {
        timeout: interval,
      });
    ...
    it("Check Solutions page with 0 solutions [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("text=No Solutions Added.", {
        timeout: interval,
      });
    ...
    it("Check Solutions have correct Descriptions [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector(".solution .description", {
        timeout: interval,
      });
    ...
    it("Check Solutions have Learn More button [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector(".solution >> text=Learn More", {
        timeout: interval,
      });
  ...
  describe("CRUD [ 50 Points ]", () => {
    describe("Create [ 12.5 Points ]", function () {
      it("Create does NOT work with empty fields [ 2.5 Points ]", async function () {
        ...
        let addCharacterBtn = await page.waitForSelector("text=Add Solution", {
          timeout: interval,
        });
      ...
      it("Create makes correct API call [ 2.5 Points ]", async function () {
        ...
        let typeElement = await page.waitForSelector('[name="type"]', {
          timeout: interval,
        });
    ...
    describe("Details [ 10 Points ]", function () {
      it("Details calls the correct API [ 2.5 Points ]", async function () {
        ...
        let learnMoreButton = await page.waitForSelector(
          `.solution:has-text("${data.description}") >> .details-btn`,
          { timeout: interval }
        );

        await Promise.all([onResponse(), learnMoreButton.click()]);
      ...
      it("Details with guest calls shows correct info [ 2.5 Points ]", async function () {
        ...
        let typeElement = await page.waitForSelector("#details-type", {
          timeout: interval,
        });
        let descriptionElement = await page.waitForSelector("#description", {
          timeout: interval,
        });
        let learnMoreElement = await page.waitForSelector("#more-info", {
          timeout: interval,
        });
    ...
    describe("Edit [ 17.5 Points ]", function () {
      ...
      it("Edit does NOT work with empty fields [ 2.5 Points ]", async function () {
        ...
        let typeInput = await page.waitForSelector('[name="type"]', {
          timeout: interval,
        });
        ...
        let learnMoreInput = await page.waitForSelector('[name="more-info"]', {
          timeout: interval,
        });
  ...
  describe("BONUS: Likes [ 15 Points ]", async () => {
    it("Likes for guests calls correct API and shows correct info [ 2.5 Points ]", async function () {
      ...
      let likesElement = await page.waitForSelector("#like", {
        timeout: interval,
      });
