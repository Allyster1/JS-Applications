  ...
  catalog: [
    {
      ...
      difficulty: "Easy",
      description:
        "Practice deep, slow breathing to reduce tension and improve focus. Inhale through your nose for 4 seconds, hold for 7 seconds, and exhale through your mouth for 8 seconds.",
    },
    {
      _id: "1002",
      _ownerId: "0002",
      ...
      difficulty: "Easy",
      description:
        "Take 10-15 minutes to walk in nature to reduce stress and improve your emotional well-being.",
    },
    {
      _id: "1001",
      _ownerId: "0001",
      ...
      difficulty: "Medium",
      description:
        "Sit quietly for 10 minutes and observe your thoughts without judgment. This helps develop mindfulness and inner peace.",
    },
  ],
};
const endpoints = {
  ...
  own: (itemId, userId) =>
    `/data/mindfultips?where=_id%3D%22${itemId}%22%20and%20_ownerId%3D%22${userId}%22&count`,
};
...
describe("E2E tests", function () {
  ...
  describe("Authentication [ 20 Points ]", function () {
    ...
    it("Login shows alert on fail and does not redirect [ 2.5 Points ]", async function () {
      ...
      let errorMessageSpanPromise = page
        .waitForSelector("#errorBox", { state: "visible", timeout: interval })
      ...
      let errorIndicator = await Promise.race([
        alertPromise,
        errorMessageSpanPromise,
      ]);
    ...
    it("Register does NOT work with different passwords [ 2.5 Points ]", async function () {
      ...
      let notificationPromise = page
        .waitForSelector("#errorBox", { state: "visible", timeout: 5000 })
        .then(() => page.$eval(".msg", (el) => el.textContent))
  ...
  describe("Navigation bar [ 10 Points ]", () => {
    it("Logged user should see correct navigation [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("nav >> text= Mindful Tips", {
        timeout: interval,
      });
    ...
    it("Guest user should see correct navigation [ 2.5 Points ]", async function () {
      ...
      expect(await page.isVisible("nav"), "Dashboard is not visible").to.be
        .true;
      expect(
        await page.isVisible("nav >> text=Share Your Tip"),
        "Create is visible"
      ).to.be.false;
    ...
    it("Guest user navigation should work [ 2.5 Points ]", async function () {
      ...
      let mindfulTipsBtn = await page.waitForSelector(
        "nav >> text=Mindful Tips",
        { timeout: interval }
      );
    ...
    it("Logged in user navigation should work [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("#tips-dashboard", { timeout: interval });
      let createBtn = await page.waitForSelector("nav >> text=Share Your Tip", {
        timeout: interval,
      });
  ...
  describe("Home Page [ 5 Points ]", function () {
    it("Show Home page text [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector(
        "text=Welcome to MindNest - your safe haven for mental wellness and mindfulness. Discover expert tips, guided meditations, and a supportive community sharing insights to help you find calm, balance, and inner peace every day. Join us and nurture your mind with care.",
        { timeout: interval }
      );
      expect(
  ...
  describe("Dashboard Page [ 15 Points ]", function () {
    it("Show Mindful Tips page - 'Mindful Tips' message [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("h3 >> text=Mindful Tips", {
        timeout: interval,
      });
    ...
    it("Check Mindful Tips page with 0 Tips [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector("text=No Mindful Tips Added Yet.", {
        timeout: interval,
      });
      expect(await page.isVisible("text=No Mindful Tips Added Yet.")).to.be
        .true;
    });
    ...
    it("Check Tips have correct Images [ 2.5 Points ]", async function () {
      ...
      const images = await page.$$eval(".tip img", (t) => t.map((s) => s.src));
    ...
    it("Check tIPS have correct Difficulty [ 2.5 Points ]", async function () {
      ...
      await page.waitForSelector(".tip-info .difficulty", {
        timeout: interval,
      });
  ...
  describe("CRUD [ 50 Points ]", () => {
    describe("Create [ 12.5 Points ]", function () {
      it("Create does NOT work with empty fields [ 2.5 Points ]", async function () {
        ...
        let addTipBtn = await page.waitForSelector("text=Share Your Tip", {
          timeout: interval,
        });
      ...
      it("Create makes correct API call [ 2.5 Points ]", async function () {
        ...
        let titleElement = await page.waitForSelector('[name="title"]', {
          timeout: interval,
        });
        let imageElement = await page.waitForSelector('[name="imageUrl"]', {
          timeout: interval,
        });
        let typeElement = await page.waitForSelector('[name="type"]', {
          timeout: interval,
        });
        let difficultyElement = await page.waitForSelector(
          '[name="difficulty"]',
          { timeout: interval }
        );
    ...
    describe("Details [ 10 Points ]", function () {
      it("Details calls the correct API [ 2.5 Points ]", async function () {
        ...
        let moreInfoButton = await page.waitForSelector(
          `.tip:has-text("${data.title}") >> .details-btn`,
          { timeout: interval }
        );
      ...
      it("Details with guest calls shows correct info [ 2.5 Points ]", async function () {
        ...
        let titleElement = await page.waitForSelector("#details-title", {
          timeout: interval,
        });
    ...
    describe("Edit [ 17.5 Points ]", function () {
      ...
      it("Edit should populate form with correct data [ 2.5 Points ]", async function () {
        ...
        const selectedDifficulty = await page.$eval(
      ...
      it("Edit does NOT work with empty fields [ 2.5 Points ]", async function () {
        ...
        let titleInput = await page.waitForSelector('[name="title"]', {
          timeout: interval,
        });
        let imageInput = await page.waitForSelector('[name="imageUrl"]', {
          timeout: interval,
        });
        let typeInput = await page.waitForSelector('[name="type"]', {
          timeout: interval,
        });
        let difficultySelect = await page.waitForSelector(
          '[name="difficulty"]',
          { timeout: interval }
        );
  ...
  describe("BONUS: Notifications [ 10 Points ]", () => {
    ...
    it("Edit notification with invalid data [ 2.5 Points ]", async () => {
      ...
      let difficultySelect = await page.waitForSelector('[name="difficulty"]', {
        timeout: interval,
      });
      let descriptionInput = await page.waitForSelector(
        '[name="description"]',
